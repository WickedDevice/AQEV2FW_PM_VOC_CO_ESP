...
#define O3_SAMPLE_BUFFER            (10)
#define NUM_SAMPLE_BUFFERS          (11)
...
#define EEPROM_CO_SENSITIVITY    (EEPROM_PM10P0_CAL_OFFSET - 4)      // float value, 4-bytes, the sensitivity from the sticker  [UNUSED]
...
#define EEPROM_CO_ZERO_NEGATIVE_RESULTS (EEPROM_CO_CAL_OFFSET - 1)
#define EEPROM_CO_BASELINE_VOLTAGE_TABLE (EEPROM_CO_ZERO_NEGATIVE_RESULTS - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_TEMPERATURE_OFFLINE_OFFSET (EEPROM_CO_BASELINE_VOLTAGE_TABLE - 4)
...
#define EEPROM_BACKUP_PM10P0_CAL_OFFSET  (EEPROM_BACKUP_CO_CAL_OFFSET + 4)
...
#define EEPROM_BACKUP_CO_SENSITIVITY     (EEPROM_BACKUP_MQTT_PASSWORD + 32)
...
void print_eeprom_value(char * arg) {
    if (strncmp(arg, "mac", 3) == 0) {
        ...
        print_eeprom_float((const float *) EEPROM_CO_CAL_OFFSET);
        Serial.print(F("    "));
        Serial.print(F("CO Report Negative values as zero: "));
        if(eeprom_read_byte((const uint8_t *) EEPROM_CO_ZERO_NEGATIVE_RESULTS)) {
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        configInject(F("restore co\r"));
        configInject(F("co_negz 1\r"));
        ...
        eeprom_write_block("aqev2_pm_voc_co_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
uint8_t collectParticulate(boolean force_reset_on_failure, boolean change_lcd_on_reset) {
    ...
    if(pm_enable_sensor_a) {
        if(!pmsx003_1.getSample(&instant_pm1p0_ugpm3_a, &instant_pm2p5_ugpm3_a, &instant_pm10p0_ugpm3_a)) {
            ...
            float c_instant_pm1p0_ugpm3_a = constrain(instant_pm1p0_ugpm3_a, 0.0f, 1000.0f);
            float c_instant_pm2p5_ugpm3_a = constrain(instant_pm2p5_ugpm3_a, 0.0f, 1000.0f);
            float c_instant_pm10p0_ugpm3_a = constrain(instant_pm10p0_ugpm3_a, 0.0f, 1000.0f);
            ...
            if( c_instant_pm1p0_ugpm3_a != instant_pm1p0_ugpm3_a ||
                ...
                pm_enable_sensor_a = 0;
                Serial.println(F("Warning: Disabling PM Sensor A because of value constraint voilation"));
            }
        }
    }
    ...
    if(pm_enable_sensor_b) {
        if(!pmsx003_2.getSample(&instant_pm1p0_ugpm3_b, &instant_pm2p5_ugpm3_b, &instant_pm10p0_ugpm3_b)) {
            ...
            float c_instant_pm1p0_ugpm3_b = constrain(instant_pm1p0_ugpm3_b, 0.0f, 1000.0f);
            float c_instant_pm2p5_ugpm3_b = constrain(instant_pm2p5_ugpm3_b, 0.0f, 1000.0f);
            float c_instant_pm10p0_ugpm3_b = constrain(instant_pm10p0_ugpm3_b, 0.0f, 1000.0f);
            ...
            if( c_instant_pm1p0_ugpm3_b != instant_pm1p0_ugpm3_b ||
                ...
                pm_enable_sensor_b = 0;
                Serial.println(F("Warning: Disabling PM Sensor B because of value constraint voilation"));
            }
        }
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"particulate\",\"voc\",\"co\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
